...
import {
  ...
  TextInput, FlatList, Modal, KeyboardAvoidingView, Platform,
} from 'react-native';
      ...
      <TouchableOpacity style={styles.matchRow} onPress={() => router.push(`/match/${m.id}`)}>
        ...
        {finished || live ? (
          <View style={[styles.scoreBox, live && styles.scoreBoxLive]}>
            ...
            <Text style={styles.scoreLabel}>{live ? 'EN VIVO' : 'FIN'}</Text>
          </View>
        ) : (
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.refreshBtn} onPress={refresh}>
          ...
          {liveMatches.length > 0 && (
            <View style={styles.liveBadge}><Text style={styles.liveBadgeText}>{liveMatches.length}</Text></View>
          )}
        </TouchableOpacity>
      </View>
      ...
      {loading && matches.length === 0 ? (
        ...
        >
          ...
          {liveMatches.map(m => {
            const score = getScore(m);
            return (
              <TouchableOpacity key={m.id} style={styles.liveRow} onPress={() => router.push(`/match/${m.id}`)}>
                <LiveBadge />
                <View style={styles.liveTeams}>
                  <TeamChip name={m.homeTeam.name ?? ''} size="sm" />
                  <Text style={styles.liveScore}>{score.home} · {score.away}</Text>
                  <TeamChip name={m.awayTeam.name ?? ''} size="sm" />
                </View>
                <View style={styles.channelBadge}><Text style={styles.channelText}>✓ Flow</Text></View>
              </TouchableOpacity>
            );
          })}
            ...
            <TouchableOpacity style={styles.hero} onPress={() => router.push(`/match/${nextMatch.id}`)}>
              ...
              <View style={styles.heroTeams}>
                <View style={styles.heroTeam}>
                  <TeamChip name={nextMatch.homeTeam.name ?? ''} size="lg" />
                  <Text style={styles.heroTeamName}>{teamName(nextMatch.homeTeam.name)}</Text>
                </View>
                <Text style={styles.vs}>vs</Text>
                <View style={styles.heroTeam}>
                  <TeamChip name={nextMatch.awayTeam.name ?? ''} size="lg" />
                  <Text style={styles.heroTeamName}>{teamName(nextMatch.awayTeam.name)}</Text>
                </View>
              </View>
              <Text style={styles.heroVenue}>🕐 {formatTime(nextMatch.utcDate)} ART</Text>
...
const styles = StyleSheet.create({
  ...
  liveRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: Colors.surface, borderRadius: 12, padding: 12, marginBottom: 8, borderWidth: 1, borderColor: Colors.liveBg },
  liveTeams: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  liveScore: { color: Colors.text, fontSize: 16, fontWeight: '800', fontVariant: ['tabular-nums'] },
  channelBadge: { backgroundColor: Colors.greenBg, borderRadius: 6, paddingHorizontal: 8, paddingVertical: 3, borderWidth: 1, borderColor: Colors.greenBorder },
  channelText: { color: Colors.green, fontSize: 11, fontWeight: '700' },
